...
testset = torchvision.datasets.CIFAR10(
    root='../../cifar10/vgg16/data', train=False, download=True, transform=transform_test)
...
def eot_grad_weights(model, x, y_target, M, sigma):
    model.zero_grad()
    for _ in range(M):
...
def compute_channel_gradients(model):
    ...
    for x, _ in loader_small:
        ...
        x[:, :, PATCH_Y:PATCH_Y + PATCH_H, PATCH_X:PATCH_X + PATCH_W] = trigger
        y = torch.full((x.size(0),), TARGET_CLASS, dtype=torch.long, device=device)
        eot_grad_weights(model, x, y, EOT_M, SIGMA)
        break
...
for k in K_VALUES:
    ...
    plt.ylabel('ASR (%)')
    fig.savefig('./result/asr_p3a_adaptive.png', bbox_inches='tight')
    if asr >= 90.0:
