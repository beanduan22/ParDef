...
def main():
    ...
    if args.arch == 'resnet32':
        ...
        from ResNet import ResNet32, ARIWrapper
        base_model = ResNet32(num_classes=num_classes, defense=False)
    else:
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'vgg16'))
        from VGG import VGG16, ARIWrapper
        base_model = VGG16(num_classes=num_classes, defense=False)
